-> dtprobe4 - datatypes probe round 4 (30.7.26): round 3's window
-> road laid out PERFECTLY (the JPEG measured its true 1440x1440 -
-> full decode!) but painted nothing: the layout is ASYNCHRONOUS,
-> and the refresh must come when the class signals completion via
-> IDCMP_IDCMPUPDATE - not immediately (round 3 refreshed into a
-> not-yet-decoded object, then went deaf). This round listens:
-> AddDTObject, then Wait on the window port; every IDCMPUPDATE ->
-> RefreshDTObjectA; hold the picture ~8s after the last update.
-> PNG control first, then the JPEG. Output: Amiga:dtprobe4.out
...
PROC windowroad(label, file)
  ...
  -> means the class has news - refresh so it can draw itself. Stop
  -> ~8s after the last update, or after ~40s with none (decode may
  -> be slow, but not endless).
  ...
  GetDTAttrsA(o,
    [DTA_TOTALVERT,  {totv},
     DTA_TOTALHORIZ, {toth},
     TAG_DONE,       NIL])
  StringF(line, '  updates: \d  totalhoriz \d totalvert \d\n',
          nupd, toth, totv)
  out(line)
...
PROC main()
  IF (fo := Open('Amiga:dtprobe4.out', NEWFILE)) = NIL THEN RETURN
  IF (datatypesbase := OpenLibrary('datatypes.library', 39)) = NIL
    out('no datatypes.library\n')
    Close(fo)
    RETURN
  ENDIF
  out('dtprobe round 4 - the listening window road\n')
  ...
  out('\ndtprobe4 done\n')
  Close(fo)
  WriteF('dtprobe4 done - output in Amiga:dtprobe4.out\n')
ENDPROC
