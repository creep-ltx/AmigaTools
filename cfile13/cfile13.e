...
-> frame for whatever font the config names (S:cfile13.config:
...
->               bits, which h/s/p/a/r/w/e/d toggle live; also its
->               icon DOpus-style (type, default tool, tooltypes -
->               t pages them)
...
MODULE 'intuition/intuition','intuition/screens',
       'graphics/text','graphics/rastport','graphics/view',
       'dos/dos','dos/dosextens',
       'devices/inputevent','diskfont',
       'graphics/gfx',
       'exec/memory','exec/ports','exec/nodes','exec/io',
       'amigalib/ports','amigalib/io',
       'icon','workbench/workbench',
       'exec/libraries'

CONST CPATHLEN=300,
      MAXENT=300,     -> CFile13 0.1b17: 500 in main CFile
      CBUFSZ=16384, PIPESZ=4096,
...
DEF enames[600]:ARRAY OF LONG,   -> entry names, MAXENT slots per pane
    edirs[600]:ARRAY OF CHAR,    -> nonzero = entry is a directory
    emark[600]:ARRAY OF CHAR,    -> nonzero = entry is marked
    esize[600]:ARRAY OF LONG,    -> file size (0 for dirs/volumes)
    edate[600]:ARRAY OF LONG,    -> sortable date key (days*1440+minute,
                                  -> 0 for archive members and volumes)
    eext[600]:ARRAY OF LONG,     -> ISO panes: the entry's extent LBA
    ...
    eddk[600]:ARRAY OF LONG,     -> C4: the datekey each memo is FOR.
    ...
    scrdepth=3,     -> CFile13 0.1b14: own-screen depth, 2 on chip-starved boxes
    ...
    cmlines=0,        -> M1: scrollback lines actually allocated -
                      -> CMAXL or 500, picked by AvailMem at the
                      -> livestart alloc (0 until a model exists)
    vbwinsz=VBWIN,    -> M2: the viewer window's real size - VBWIN
                      -> or 64K, picked by AvailMem once at startup
    ...
    rc=0,
    -> R12 (KS1.3): the keyboard road. A 1.3 window that asks for
    -> VANILLAKEY never sees the keys that do not translate to one
    -> character (arrows, F-keys, Help) - the "untranslatable keys
    -> fall through as RAWKEY" rule is V36. So the windows ask for
    -> RAWKEY only and CFile translates keypresses itself through
    -> console.device's RawKeyConvert (unit CONU_LIBRARY: no console,
    -> just the device's library vectors) - the user's real keymap
    -> (setmap s) decides, dead keys included.
    conport=NIL:PTR TO mp,        -> reply port for the device open
    condev=NIL:PTR TO iostd,      -> the open request itself
    conbase=NIL,                  -> device base = library base (JSR -48)
    conie=NIL:PTR TO inputevent,  -> the one inputevent rkconv refills
    imcode=0, imqual=0,           -> the decoded key waitim/pollim return
    conkbuf[8]:ARRAY OF CHAR      -> RawKeyConvert's output buffer
...
PROC initpanes()
  ...
  -> CFile13 (0.1b14, retuned 0.1b16): the 512K rung - where 64K is
  -> an eighth of everything, 8K is the right size (the floppy's
  -> 11K/s hides any number of Write round trips)
  IF AvailMem(0) < $100000 THEN cbufsz := 8192
  ...
  -> M2 (the 1MB pass): the viewer's 256KB sliding window is a
  -> quarter of a 1MB machine - the copybuf ladder's treatment, set
  -> ONCE here. The buffer itself stays New()'d per view (raw bytes;
  -> String() would cap at 32K anyway). VBWIN stays the ceiling.
  vbwinsz := IF AvailMem(0) > $200000 THEN VBWIN ELSE 65536
  IF AvailMem(0) < $100000 THEN vbwinsz := 16384    -> 0.1b14: 512K rung
ENDPROC
...
-> would put up a "please insert volume" requester. R9 (KS1.3): the
-> Forbid snapshot walk replaces LockDosList/FindDosEntry; all three
-> node types count, like the old LDF_ASSIGNS|DEVICES|VOLUMES ask.
PROC haveassign(name)
  DEF snap=NIL:PTR TO CHAR, n, k, found=FALSE
  IF (snap := New(100 * 36)) = NIL THEN RETURN FALSE
  n := doslistsnap(snap, 100)
  FOR k := 0 TO n - 1
    IF nccmp(snap + Mul(k, 36) + 4, name) = 0 THEN found := TRUE
  ENDFOR
  Dispose(snap)
ENDPROC found

-> CFile13 (0.1b9): an Open that never ASKS. Probing a device that
-> may not exist (PIPE: is mounted by default on 3.x, absent on a
-> stock 1.3 - his pack test raised "Please insert volume PIPE")
-> must fail to NIL so the caller's fallback road runs; without
-> this, dos pops the insert-volume requester first. The classic
-> discipline: park -1 in the process WindowPtr for the call.
PROC quietopen(name, mode)
  DEF pr:PTR TO process, oldw, fh
  pr := FindTask(NIL)
  oldw := pr.windowptr
  pr.windowptr := -1
  fh := Open(name, mode)
...
-> AssignLock is V36; on 1.3 the only public road to an assign is
-> C:Assign (it splices the DosList node itself). Execute runs it
-> synchronously; the re-check afterwards is the honest success test.
PROC ensureassigns()
  DEF lock
  IF haveassign('ENV') = FALSE
    IF lock := dirlock('RAM:Env')
      UnLock(lock)
      Execute('assign ENV: RAM:Env', NIL, NIL)
      IF haveassign('ENV') THEN madeenv := TRUE
    ENDIF
  ENDIF
  IF haveassign('T') = FALSE
    IF lock := dirlock('RAM:T')
      UnLock(lock)
      Execute('assign T: RAM:T', NIL, NIL)
      IF haveassign('T') THEN madet := TRUE
    ENDIF
  ENDIF
ENDPROC
...
PROC dropassigns()
  IF madeenv THEN Execute('assign ENV:', NIL, NIL)
  IF madet THEN Execute('assign T:', NIL, NIL)
ENDPROC

-> ---- KS1.3 compatibility helpers -----------------------------------
-> The 2.0+ dos conveniences this fork cannot call, rebuilt on V33
-> roads. Everything below exists so the rest of the file reads as
-> before.

-> R12 (KS1.3): open console.device as a keymap library - unit -1
-> (CONU_LIBRARY) opens no console at all, only the device's
-> library-callable vectors, RawKeyConvert among them. The port and
-> iorequest come from amigalib (plain E code, Kickstart-independent,
-> the CBoot road). If the open fails - a 1.3 without console.device
-> cannot happen - conbase stays NIL and openui falls back to asking
-> for VANILLAKEY exactly as 0.1b3 did.
PROC conopen()
  DEF ok=FALSE
  IF (conie := New(SIZEOF inputevent)) = NIL THEN RETURN
  IF (conport := createPort(0, 0))
    IF (condev := createExtIO(conport, SIZEOF iostd))
      IF OpenDevice('console.device', -1, condev, 0) = FALSE
        conbase := condev.device
        ok := TRUE
      ENDIF
    ENDIF
  ENDIF
  IF ok = FALSE THEN conclose()
ENDPROC
...
-> one raw downstroke through the loaded keymap: chars land in
-> conkbuf, the count comes back (1 = a vanilla keypress; 0 = dead
-> key, >1 = a CSI sequence like the arrows, -1 = overflow - all
-> "not vanilla", the caller keeps those RAWKEY). eventaddress: a
-> RAWKEY intuimessage's IAddress points AT a pointer to the
-> dead-key data - dereference once, guard NIL. RawKeyConvert is
-> LVO -48 off the console base; E has no stub for calling a device
-> as a library, so the JSR is spelled out (A6 saved around it, A2
-> = NIL keymap = the user's current default).
PROC rkconv(rcode, rqual, ria)
  DEF res
  conie.class := IECLASS_RAWKEY
  conie.subclass := 0
  conie.code := rcode
  conie.qualifier := rqual
  conie.eventaddress := IF ria THEN Long(ria) ELSE NIL
  MOVE.L A6,-(A7)
  MOVE.L conie,A0
  MOVE.L conkbuf,A1
  MOVEQ #8,D1
...
-> the input shim: every key loop reads through waitim (blocking) or
-> pollim (non-blocking) and sees 2.0 semantics on any Kickstart - a
-> RAWKEY downstroke that maps to exactly one character comes back
-> as the VANILLAKEY a 2.0 window would have sent, everything else
-> (arrows, F-keys, dead keys, key-ups) passes through as RAWKEY.
-> The event rides imcode/imqual; imqual is always the ORIGINAL
-> qualifier - branches test Shift/Ctrl on both classes.
PROC transkey(class, code, qual, ia)
  imcode := code
  imqual := qual
  IF class = IDCMP_RAWKEY
    IF conbase
      IF code < $80    -> downstrokes only; key-ups stay raw
        IF rkconv(code, qual, ia) = 1
          imcode := conkbuf[0]
...
ENDPROC transkey(class, MsgCode(), MsgQualifier(), MsgIaddr())

-> -1 = nothing pending, else the (translated) class
PROC pollim(w:PTR TO window)
  DEF msg:PTR TO intuimessage, class, code, qual, ia
  IF (msg := GetMsg(w.userport)) = NIL THEN RETURN -1
  class := msg.class
  code := msg.code
  qual := msg.qualifier
  ia := msg.iaddress
  ReplyMsg(msg)
ENDPROC transkey(class, code, qual, ia)

-> AllocDosObject(DOS_FIB) is V36. A fileinfoblock only needs 260
-> zeroed, longword-aligned bytes - E's New() guarantees all three.
PROC allocfib() IS New(SIZEOF fileinfoblock)

PROC freefib(fib)
  IF fib THEN Dispose(fib)
ENDPROC

-> AddPart/FilePart/PathPart are V36 dos calls; these cover exactly
-> the shapes this file uses (plain child names joined onto E-string
-> paths - never absolute names replacing the base).
PROC addpart(dst, name, max)    -> max kept for the callers' shape;
  DEF l, s:PTR TO CHAR          -> E-string StrAdd clips at dst's cap
  l := EstrLen(dst)
  s := dst
  IF l > 0
  ...
  ENDIF
  StrAdd(dst, name)
  IF EstrLen(dst) > max THEN RETURN FALSE    -> the V36 overflow answer
ENDPROC TRUE

-> pointer to the last path component (after the last '/' or ':')
PROC filepart(path)
  DEF s:PTR TO CHAR, i=0, last=0
  s := path
  WHILE s[i]
    IF (s[i] = "/") OR (s[i] = ":") THEN last := i + 1
    i++
  ENDWHILE
ENDPROC s + last

-> Fault is V36; this table covers the errors a file manager
-> actually meets (the classic texts, so messages read as before),
-> and everything else says its number honestly.
PROC faultname(err)
  SELECT err
    CASE 103;  RETURN 'no free store'
    CASE 202;  RETURN 'object is in use'
    CASE 203;  RETURN 'object already exists'
    CASE 204;  RETURN 'directory not found'
    CASE 205;  RETURN 'object not found'
    CASE 210;  RETURN 'invalid stream component name'
    CASE 211;  RETURN 'invalid object lock'
    CASE 212;  RETURN 'object not of required type'
    CASE 213;  RETURN 'disk not validated'
    CASE 214;  RETURN 'disk is write-protected'
    CASE 215;  RETURN 'rename across devices attempted'
    CASE 216;  RETURN 'directory not empty'
    CASE 217;  RETURN 'too many levels'
    CASE 218;  RETURN 'device or volume is not mounted'
    CASE 219;  RETURN 'seek failure'
    CASE 220;  RETURN 'comment is too long'
    CASE 221;  RETURN 'disk is full'
    CASE 222;  RETURN 'object is protected from deletion'
    CASE 223;  RETURN 'file is write protected'
    CASE 224;  RETURN 'file is read protected'
    CASE 225;  RETURN 'not a valid DOS disk'
    CASE 226;  RETURN 'no disk in drive'
    CASE 232;  RETURN 'no more entries in directory'
  ENDSELECT
ENDPROC NIL

-> SetFileDate is V36; on 1.3 the date rides ACTION_SET_DATE (34)
-> straight to the handler, the way C:SetDate itself did it. One
-> packet, sent synchronously over OUR process port - CFile never
-> has another dos packet of its own in flight, so the reply cannot
-> interleave with anything. Arg1 NULL, Arg2 the lock the name is
-> relative to (DeviceProc's IoErr side channel), Arg3 the path as
-> a BSTR, Arg4 the datestamp. A path over 255 chars cannot ride a
-> BSTR - the date is then simply not copied, the same silence
-> copyattribs already keeps for every other failure.
PROC setfiledate13(name, stamp)
  DEF port, dlock, sp=NIL:PTR TO standardpacket, bn=NIL:PTR TO CHAR,
      pr:PTR TO process, l, i, s:PTR TO CHAR, res=FALSE
  l := StrLen(name)
  IF l > 255 THEN RETURN FALSE
  IF (port := DeviceProc(name)) = NIL THEN RETURN FALSE
  dlock := IoErr()
  sp := New(SIZEOF standardpacket)
  bn := New(l + 8)    -> New is longword-aligned: bn/4 is a clean BPTR
  IF (sp = NIL) OR (bn = NIL)
    IF sp THEN Dispose(sp)
    IF bn THEN Dispose(bn)
    RETURN FALSE
  ENDIF
  s := name
  bn[0] := l
  ...
  ENDFOR
  pr := FindTask(NIL)
  sp.msg.ln.name := sp.pkt      -> handlers find the packet here
  sp.pkt.link := sp.msg
  sp.pkt.port := pr.msgport     -> the reply lands on our port
  sp.pkt.type := ACTION_SET_DATE
  sp.pkt.arg1 := NIL
  sp.pkt.arg2 := dlock
  sp.pkt.arg3 := Shr(bn, 2)
  sp.pkt.arg4 := stamp
  PutMsg(port, sp.msg)
  WaitPort(pr.msgport)
  GetMsg(pr.msgport)
...
-> R5 (KS1.3): the V33 detach. SYS_ASYNCH is V36, and a plain
-> "run <cmd> ><file>" line is a shared-filehandle trap on 1.3: the
-> redirection would be opened by the INVOKING CLI and closed when
-> run returns, under the still-running child (1.3 filehandles have
-> no refcount). The safe road is a one-line T: script - the
-> BACKGROUND CLI itself opens and closes the redirection, so the
-> capture file/pipe is held exactly as long as the child runs
-> (arcpollrun's exclusive-Lock completion test rides on that, and
-> a PIPE: reader sees EOF exactly at child exit). The child
-> inherits the caller's CWD lock UNSHARED-UNPROTECTED (again no
-> refcount), so callers must hold their CurrentDir lock until the
-> child provably exited. Returns 0 (launched) or -1, the old
-> SYS_ASYNCH result shape. Needs C:Run and C:Execute - both live
-> on every stock 1.3 boot disk.
PROC runasync(cmd, outfile)
  DEF fh=NIL, line[820]:STRING, i
  -> a just-finished run's CLI can still hold the script for a beat
  -> (the capture file closes before C:Execute lets the script go):
  -> retry briefly instead of failing a batched caller's next round
  FOR i := 1 TO 10
    IF fh = NIL
      fh := Open('T:CFile-run', NEWFILE)
      IF fh = NIL THEN Delay(2)
    ENDIF
  ENDFOR
  IF fh = NIL THEN RETURN -1
  StringF(line, '\s >\s\n', cmd, outfile)
  IF Write(fh, line, EstrLen(line)) <> EstrLen(line)
    Close(fh)
    RETURN -1
  ENDIF
  Close(fh)
  IF Execute('run execute T:CFile-run', NIL, NIL) = FALSE THEN RETURN -1
ENDPROC 0

-> LockDosList/NextDosEntry/FindDosEntry are V36. On 1.3 the device
-> list hangs off dosbase itself: dl_Root (a real pointer) ->
-> RootNode.rn_Info (BPTR) -> DosInfo.di_DevInfo (BPTR) -> a chain
-> of DosList nodes linked by dol_Next (BPTR). dol_Type: DLT_DEVICE
-> 0, DLT_DIRECTORY 1 (an assign), DLT_VOLUME 2. dol_Name is a BPTR
-> to a BCPL string - length byte first, chars after, NO NUL.
-> The only lock is Forbid(): the walk SNAPSHOTS type+name into the
-> caller's buffer and Permits before anything else happens - no DOS
-> call may ever run inside the window (the handler the packet would
-> go to is one of the very processes Forbid froze).
-> Snapshot slots are 36 bytes: LONG type + a NUL-terminated name
-> (clipped to 31 chars, the BSTR maximum anyway). Returns the count.
PROC doslistsnap(buf:PTR TO CHAR, maxsl)
  DEF db:PTR TO doslibrary, rn:PTR TO rootnode, di:PTR TO dosinfo,
      dl:PTR TO doslist, s:PTR TO CHAR, p2:PTR TO CHAR, n=0, len, i
  db := dosbase
  Forbid()
  rn := db.root
  di := Shl(rn.info, 2)
  dl := Shl(di.devinfo, 2)
  WHILE (dl <> NIL) AND (n < maxsl)
    p2 := buf + Mul(n, 36)
    PutLong(p2, dl.type)
    s := Shl(dl.name, 2)
    len := s[0]
...
-> NameFromLock is V36; the V33 road walks the parent chain itself.
-> Examine names each level (on the volume root it names the VOLUME,
-> which is the canonicalisation gotopath wants), ParentDir climbs,
-> and the path assembles back-to-front: root ends with ':',
-> components join with '/'. Every intermediate lock is UnLocked;
-> the caller's lock is only DupLock'd, never touched. dst is an
-> E-string - a path deeper than its cap clips (StrCopy's own rule),
-> same as NameFromLock's buffer-too-small case would have failed.
PROC locknamefrom(lock, dst)
  DEF l, pl, fib:PTR TO fileinfoblock, tmp[CPATHLEN]:STRING, ok=TRUE
  StrCopy(dst, '')
  IF (fib := allocfib()) = NIL THEN RETURN FALSE
  l := DupLock(lock)
  IF l = NIL
    freefib(fib)
    RETURN FALSE
  ...
  WHILE l
    ...
    ELSE
      ...
      UnLock(l)
      IF pl
        -> a directory level: "<name>/<what we have>" (no slash yet
        -> when dst is still empty - the lock's own level)
        StrCopy(tmp, fib.filename)
        IF EstrLen(dst) > 0 THEN StrAdd(tmp, '/')
      ELSE
        -> the volume root: "<volume>:<what we have>"
        StrCopy(tmp, fib.filename)
        StrAdd(tmp, ':')
      ENDIF
      StrAdd(tmp, dst)
...
-> SameLock is V36. A lock is a BPTR to a filelock; equal fl_Task
-> (the handler) AND equal fl_Key (the object) is exactly what
-> LOCK_SAME reported, and equal fl_Task alone is the same-volume
-> question. NIL guards are nested IFs - E's AND does not
-> short-circuit and Shl(NIL,2) would still be dereferenced.
PROC locksame(a, b)             -> TRUE = the same object
  DEF fa:PTR TO filelock, fb:PTR TO filelock
  IF a = NIL THEN RETURN FALSE
  IF b = NIL THEN RETURN FALSE
  fa := Shl(a, 2)
  fb := Shl(b, 2)
  IF fa.task = fb.task
    IF fa.key = fb.key THEN RETURN TRUE
  ENDIF
ENDPROC FALSE

PROC locksamevol(a, b)          -> TRUE = the same volume/handler
  DEF fa:PTR TO filelock, fb:PTR TO filelock
  IF a = NIL THEN RETURN FALSE
  IF b = NIL THEN RETURN FALSE
  fa := Shl(a, 2)
  fb := Shl(b, 2)
ENDPROC fa.task = fb.task

-> DateToStr is V36; the fork formats dos datestamps itself. A
-> datestamp is days since 1.1.1978 / minutes into the day / ticks
-> (1/50s). Split by subtract-per-year: no 16-bit DIVS exposure at
-> all, and ~50 iterations is nothing on a cold path. Leap = every
-> 4th year ((y AND 3) = 0 is exact for 1901-2099; 2100 is out of
-> any Amiga's reach and ignored, per the classic dos rule).
PROC dssplit(days, yp:PTR TO LONG, mp:PTR TO LONG, dp:PTR TO LONG)
  DEF y, m, yl, ml, md:PTR TO LONG
  md := [31,28,31,30,31,30,31,31,30,31,30,31]
  IF days < 0 THEN days := 0
  y := 1978
  yl := 365    -> 1978 is no leap year
  WHILE days >= yl
    days := days - yl
    y++
    yl := IF (y AND 3) = 0 THEN 366 ELSE 365
  ENDWHILE
  m := 1
  ml := 31
  WHILE days >= ml
...
-> "DD-MMM-YY", exactly what DateToStr FORMAT_DOS printed (StringF
-> writes to plain arrays and E-strings alike; \d[2] zero-pads)
PROC dsdatestr(days, dst)
  DEF y, m, d
  dssplit(days, {y}, {m}, {d})
  StringF(dst, '\d[2]-\s-\d[2]', d, monthname(m), Mod(y, 100))
ENDPROC

-> "HH:MM:SS" from the stamp's minutes + ticks (both under 16 bits,
-> so Div/Mod are safe here)
PROC dstimestr(mins, ticks, dst)
  StringF(dst, '\d[2]:\d[2]:\d[2]', Div(mins, 60), Mod(mins, 60),
          Div(ticks, 50))
ENDPROC

-> pointer to where the directory part ends: AT the last '/', or just
-> past the ':' when there is no slash (the V36 PathPart contract)
PROC pathpart(path)
  DEF s:PTR TO CHAR, i=0, sl=-1, co=-1
  s := path
  WHILE s[i]
    IF s[i] = "/" THEN sl := i
    IF s[i] = ":" THEN co := i
    i++
  ENDWHILE
  IF sl >= 0 THEN RETURN s + sl
  IF co >= 0 THEN RETURN s + co + 1
ENDPROC s
...
-> the config lives in S:cfile13.config (CFile13: PROGDIR is a V36
-> assign - on 1.3 an Open of it raises the insert-volume requester
-> HIS FIRST 1.3 BOOT hit; S: exists on every boot disk).
...
PROC loadconfig()
  ...
  StrCopy(cfgfont, '')
  IF (fh := Open('S:cfile13.config', OLDFILE)) = NIL THEN RETURN
  buf := slurpfh(fh, {n})
  Close(fh)
...
PROC saveconfig()
  ...
  IF fh := Open('S:cfile13.config', OLDFILE)
    buf := slurpfh(fh, {n})
    Close(fh)
  ENDIF
  ...
  IF n < 0 THEN RETURN
  IF (fh := Open('S:cfile13.config', NEWFILE)) = NIL
...
PROC configensure()
  ...
  IF fh := Open('S:cfile13.config', MODE_OLDFILE)
    exists := TRUE
    buf := slurpfh(fh, {n})
    Close(fh)
  ENDIF
  ...
  IF exists = FALSE
    ...
    IF fh := Open('S:cfile13.config', MODE_NEWFILE)
...
-> CFile13: ADF mounting/creation and DMS unpacking cut (DAControl and
-> trackfile.device are AmigaOS 3.2) - .adf and .dms are plain files here
...
-> compact day+month for the size column when sorting by date: the
-> "DD-Mon-YY" shape minus dashes, spaces and year = "DDMon" (max
-> 5 chars, same slot as a size). datekey = days*1440 + minute.
-> R6 (KS1.3): formatted by dssplit now, same five characters. The
-> Div is safe: days ~17,700 in 2026, far under the 16-bit quotient.
PROC datestr(datekey, dst)
  DEF y, m, d
  dssplit(Div(datekey, 1440), {y}, {m}, {d})
  StringF(dst, '\d[2]\s', d, monthname(m))
ENDPROC
...
-> R5 (KS1.3): Execute (V33) replaces SystemTagList. Execute is
-> synchronous with a NIL input and returns only a DOS boolean - the
-> command's OWN return code does not exist on 1.3. So: 0 = it ran,
-> -1 = it could not start; every caller already verifies the EFFECT
-> (parsed output, the result file's existence), which is why the
-> RC's loss only costs the old "returned code n" courtesy notes.
PROC runcapture(dir, cmd, outfile)
  DEF dlock=NIL, old=NIL, res=-1, fout=NIL
  IF dir
  ...
  IF fout
    IF Execute(cmd, NIL, fout) THEN res := 0
    -> (the b10 runner diagnostics lived here while the 1.3 pack
    -> road was hunted down - the trail is in PORT-STATUS.md)
    Close(fout)
  ENDIF
  IF dlock
    CurrentDir(old)
...
-> the directory iterator. On the 2.0 line this batched with ExAll
-> (0.4.1 I3); ExAll is V36, so the 1.3 fork runs the fallback road
-> ONLY: Examine once, then one ExNext round trip per entry - the
-> honest V33 cost. The iterator's shape survives untouched so every
-> walker reads as before: state lives in the CALLER's frame
-> (DEF s[1]:ARRAY OF eascan) so recursive walkers can nest scans,
-> and each entry is shaped into one reusable easentry. E note: no
-> short-circuit ANDs in any loop body - readdir's own old rule.
OBJECT easentry              -> what ExAll's exalldata used to carry,
  name:PTR TO CHAR           -> refilled per entry from the scan's fib
  type:LONG
  size:LONG
  days:LONG, mins:LONG, ticks:LONG
  comment:PTR TO CHAR
ENDOBJECT
...
OBJECT eascan
  ...
  fib:PTR TO fileinfoblock      -> New(260): zeroed, longword-aligned
  fbed:PTR TO easentry          -> the reusable entry
ENDOBJECT
...
PROC easnext(s:PTR TO eascan)
  DEF ed:PTR TO easentry, ds:PTR TO LONG
  IF s.err THEN RETURN NIL
  IF ExNext(s.lock, s.fib) = FALSE
    -> ERROR_NO_MORE_ENTRIES is the clean end; anything else died
...
PROC readdir(p)
  DEF lock=NIL, fib=NIL:PTR TO fileinfoblock,
      s[1]:ARRAY OF eascan, sc:PTR TO eascan, ed:PTR TO easentry
  ecount[p] := 0
  efail[p] := FALSE
  clearmarks(p)
...
-> R9 (KS1.3): the Forbid snapshot walk replaces LockDosList/
-> NextDosEntry - names are copied out under Forbid, the pane fills
-> from the snapshot afterwards. Two passes keep the old shape:
-> volumes first, assigns below (tier 1 sorts after tier 255);
-> devices are not listed, exactly as before.
PROC readvolumes(p)
  DEF snap=NIL:PTR TO CHAR, n, k, e:PTR TO CHAR, nm[34]:STRING
  ecount[p] := 0
  efail[p] := FALSE
  clearmarks(p)
  IF (snap := New(100 * 36)) = NIL
    efail[p] := TRUE
    RETURN
  ENDIF
  n := doslistsnap(snap, 100)
  ...
  FOR k := 0 TO n - 1
    e := snap + Mul(k, 36)
    IF Long(e) = DLT_DIRECTORY    -> dol_Type 1 = an assign
      StrCopy(nm, e + 4)
      StrAdd(nm, ':')
      addentry(p, nm, 1, 0, 0)
    ENDIF
...
-> CFile13: auto-refresh cut (KS1.3 filesystems have no StartNotify);
-> F5 stays the manual rescan road
...
PROC openui()
  DEF wbs:PTR TO screen, vm, sw, sh, idcmp
  openfont()
  -> R12 (KS1.3): RAWKEY only - the shim (waitim/pollim) synthesizes
  -> the vanilla keys through RawKeyConvert. Only if console.device
  -> failed to open (it cannot) ask for VANILLAKEY as 0.1b3 did.
  conopen()
  idcmp := IF conbase THEN IDCMP_RAWKEY ELSE (IDCMP_RAWKEY OR IDCMP_VANILLAKEY)
  -> R3 (KS1.3): OpenScreenTagList is V36. GetScreenData (V33) asks
  -> the Workbench screen for its size and viewmodes - the same
  -> numbers SA_LIKEWORKBENCH copied - and OpenScreen takes a
  -> NewScreen. Depth 3, the quiet/no-title look and the pen setup
  -> downstream stay exactly as the tag road had them. SA_PUBNAME
  -> died with the pubscreen system itself (V36) - on 1.3 no console
  -> can visit the screen, so the PubScreenStatus call went too.
  scr := NIL
  -> CFile13 (0.1b14): the 512K rung. A chip-starved machine gets a
  -> depth-2 screen: 40K of chip instead of 60K, and every blit a
  -> third faster on a bus the CPU shares with the display. The
  -> pens remap below, and the 3.x plane-mask trick (depth-3 pens
  -> by construction) keys off scrdepth as well.
  scrdepth := IF AvailMem(MEMF_CHIP) < $40000 THEN 2 ELSE 3
  IF (wbs := New(SIZEOF screen))
    IF GetScreenData(wbs, SIZEOF screen, WBENCHSCREEN, NIL)
      sw := wbs.width
      sh := wbs.height
      vm := wbs.viewport.modes
      scr := OpenScreen([0, 0, sw, sh, scrdepth,
                         0, 1,            -> detail/block pen
                         vm,              -> the WB clone's viewmodes
                         CUSTOMSCREEN OR SCREENQUIET,
                         NIL, 'CFile', NIL, NIL]:ns)
    ENDIF
    Dispose(wbs)
  ENDIF
  IF scr
    ownscr := TRUE
    ShowTitle(scr, FALSE)          -> what SA_SHOWTITLE FALSE said
    win := OpenWindow([0, 0, scr.width, scr.height, 255, 255,
      idcmp,
      WFLG_BACKDROP OR WFLG_BORDERLESS OR WFLG_ACTIVATE OR WFLG_RMBTRAP,
      NIL, NIL, NIL, scr, NIL, 0, 0, 0, 0, CUSTOMSCREEN]:nw)
  ELSE
    -> fallback: a window on the Workbench screen, no palette of our
    -> own. R4 (KS1.3): LockPubScreen/PubScreenStatus are V36 - a
    -> plain WBENCHSCREEN-type NewWindow replaces the whole pubscreen
    -> road, sized by GetScreenData like the WA_WIDTH tags were from
    -> the locked screen. ownscr stays FALSE, so the mask rule
    -> ("never mask a foreign screen", masks at 255) holds unchanged.
    ownscr := FALSE
    panemask := 255    -> R6: never mask a foreign screen
    flatmask := 255
    ...
    IF (wbs := New(SIZEOF screen))
      ...
      Dispose(wbs)
    ENDIF
    win := OpenWindow([0, 0, sw, sh, 255, 255,
      idcmp,
      WFLG_BORDERLESS OR WFLG_ACTIVATE OR WFLG_RMBTRAP,
      NIL, NIL, NIL, NIL, NIL, 0, 0, 0, 0, WBENCHSCREEN]:nw)
    scr := NIL
  ENDIF
  ...
  IF ownscr
    ...
    IF scrdepth = 3
      ...
      errpen := 5
    ELSE
      dirpen := 2    -> depth-2: green dirs, yellow errors
      errpen := 3
    ENDIF
    ...
    IF (gfxversion() >= 39) AND (scrdepth = 3)
...
PROC setlightpal()
  IF ownscr
    ...
    ELSE
      -> CFile13 (0.1b15): depth-2 wears the SAME clothes - colours
      -> 2/3 get the exact blue/magenta that pens 4/5 wear on the
      -> full screen (his 512K boot: dirs came up in colour 2's
      -> leftover green - "super ugly" - for no reason at all)
      LoadRGB4(ViewPortAddress(win),
        [$0AAA,$0000,$055E,$0D2D]:INT, 4)
    ENDIF
  ENDIF
ENDPROC
...
PROC setansipal()
  IF ownscr
    ...
    ELSE
      -> depth-2: the first four ANSI colours are at least TRUE
      -> (black/red/green/yellow); pens 4-7 alias down - the fold
      -> is the 4-colour screen's honest limit
      LoadRGB4(ViewPortAddress(win),
        [$0000,$0E22,$02C2,$0EE2]:INT, 4)
    ENDIF
  ENDIF
ENDPROC
...
PROC closeui()
  ...
  ENDIF
  IF scr
    IF ownscr THEN CloseScreen(scr)    -> fallback keeps scr NIL (R4)
    scr := NIL
  ENDIF
  IF tf
    CloseFont(tf)
    tf := NIL
  ENDIF
  conclose()    -> R12: give console.device back (guarded, idempotent)
ENDPROC
...
-> CFile13 (0.1b5): TRUE when the path's volume head is the RAM
-> disk. The ram-handler - 1.3 and 3.x alike - reports itself
-> permanently 100% full (used = total) because it grows on demand;
-> taken literally that shows "0K free" AND makes the copy precheck
-> refuse every copy INTO RAM: (his 512K first-boot find). The
-> handler hardcodes its volume name, so the name IS the signature.
PROC isramdisk(path:PTR TO CHAR)
  DEF hd[40]:STRING, i=0
  WHILE (path[i] <> 0) AND (path[i] <> ":") AND (i < 38)
    hd[i] := path[i]
    i++
  ENDWHILE
  SetStr(hd, i)
  -> both roads to the same handler: "RAM DISK:" (the volume, what
  -> the volume list hands you) and "RAM:" (the device, what a
  -> config start path or g says) - his boot showed one working
  -> and the other stuck at 0K
ENDPROC (nccmp(hd, 'Ram Disk') = 0) OR (nccmp(hd, 'RAM') = 0)
...
PROC freebytes(p)
  ...
  IF lock := Lock(ppath[p], SHARED_LOCK)
    IF Info(lock, id)
      ...
      IF (nfree = 0) AND (id.numblocks = id.numblocksused) AND (isramdisk(ppath[p]))
        -> the RAM disk's real free space is free memory (see
        -> isramdisk above) - the one thing Workbench never told you
        n := AvailMem(0)
      ELSEIF bpb < 1
        n := -1
      ELSE
...
-> CFile13 (0.1b6): the 7MHz road. refreshall's drawframe opens with
-> a full-screen RectFill and repaints every art row - an invisible
-> blink on a fast machine, a visible whole-screen wipe on a stock
-> A500 (his find, first floppy session: copy and delete "redraw the
-> entire screen", rename does not - rename rides refreshpane). The
-> transfer verbs' real debris is exactly the 3-row progress box
-> (progshow: 33 cells centered, straddling the divider art) plus
-> whatever prompt sat on the border row. So: re-read both panes,
-> restore the THREE art rows the box crossed (frow is 4 cells per
-> content row), redraw pane content and the paths row - the frame,
-> the margins and every other row keep their pixels. refreshall
-> stays for surfaces that truly painted everywhere (editor, viewer,
-> help), and boxed=FALSE skips even the frow calls when no box was
-> ever shown.
...
-> R11 (KS1.3): Fault is V36 - faultname's table serves the common
-> errors, the rest print their number
PROC showfault(prefix, err)
  DEF mb[120]:STRING, s
  s := faultname(err)
  IF s
    StringF(mb, '\s: \s', prefix, s)
  ELSE
    StringF(mb, '\s: DOS error \d', prefix, err)
  ENDIF
  showmsg(mb)
ENDPROC
...
PROC lineinput(prompt, buf, max, names)
  ...
  WHILE res = -2
    ...
    ELSEIF class = IDCMP_RAWKEY
      IF code < $80
        IF code = RK_LEFT
          IF imqual AND (IEQUALIFIER_LSHIFT OR IEQUALIFIER_RSHIFT)
...
-> volume name). R8 (KS1.3): locksame's fl_Task+fl_Key compare IS
-> what SameLock(LOCK_SAME) reported.
PROC samefile(a, b)
  DEF la, lb, same=FALSE
  IF la := Lock(a, SHARED_LOCK)
    IF lb := Lock(b, SHARED_LOCK)
      IF locksame(la, lb) THEN same := TRUE
      UnLock(lb)
    ENDIF
    UnLock(la)
  ENDIF
...
PROC copyattribs(src, dst)
  ...
  IF (fib := allocfib()) = NIL THEN RETURN
  IF lock := Lock(src, SHARED_LOCK)
    IF Examine(lock, fib)
      UnLock(lock)
      SetProtection(dst, fib.protection)
      setfiledate13(dst, fib.datestamp)    -> R11: SetFileDate is V36
...
PROC checkabort()
  ...
  WHILE (class := pollim(win)) <> -1
    IF (class = IDCMP_VANILLAKEY) AND (imcode = 27) THEN abort := TRUE
  ENDWHILE
ENDPROC abort

-> the Esc-cancel family for archiver children (b21). On the 2.0 line
-> the async runs launched the child under NP_NAME = arcname(), a name
-> unique to THIS CFile instance, so the break could never land in
-> another instance's run. R5 (KS1.3): the detach is C:Run now, which
-> names its child itself - FindTask(arcname()) MISSES and arcbreak
-> degrades to a safe no-op. Esc cannot stop a running archiver on
-> 1.3; every poll/pump loop still rides to the child's own exit, so
-> nothing hangs - the break just stopped arriving. The machinery
-> stays for the day a named detach comes back.
PROC arcname()
...
PROC arcbreak()
  DEF t
  Forbid()
  t := FindTask(arcname())    -> on 1.3 this MISSES (see arcname) -
  IF t THEN Signal(t, SIGBREAKF_CTRL_C)    -> a safe no-op
  Permit()
ENDPROC
...
-> into res[], each a String'd full path, up to FINDMAX. Matching is a
-> case-insensitive substring of `pat` (CFile13: the #? glob road went
-> with pattern marking - no ParsePattern on this fork). cnt^ is shared
-> across the recursion. Cancellable via Esc (checkabort - the caller
-> sets cancelok). Heap path buffers, like copytree: a deep tree cannot
-> sit on E's stack.
PROC findwalk(dir, depth, pat, res:PTR TO LONG, cnt:PTR TO LONG)
  DEF lock=NIL, child=NIL, full=NIL, s, match,
      es[1]:ARRAY OF eascan, sc:PTR TO eascan, ed:PTR TO easentry
  IF depth > 20 THEN RETURN
  ...
  IF lock := Lock(dir, SHARED_LOCK)
    ...
    WHILE (ed <> NIL) AND (cnt[0] < FINDMAX) AND (checkabort() = FALSE)
      ...
      IF (ed.type > 0) AND (cnt[0] < FINDMAX)
        StrCopy(child, full)
        findwalk(child, depth + 1, pat, res, cnt)
      ENDIF
      ed := easnext(sc)
    ENDWHILE
    easend(sc)
...
-> parent chain comparing locks (locksame - the R8 field compare).
PROC insidedir(a, b)
...
PROC arcrunprog(dir, cmd)
  DEF wout=NIL, rdr=NIL, dlock=NIL, old=NIL, res,
      buf:PTR TO CHAR, n, i, c, st=0, prevtotal=0, curtot=0, memdone=0,
      killed=FALSE, w
  buf := pipebuf    -> I7a: 4KB shared pipe buffer, not 256B on the stack
  IF (wout := quietopen('PIPE:cfile-arc', NEWFILE)) = NIL
    RETURN runcapture(dir, cmd, 'T:CFile-out')    -> no PIPE: no bar
  ENDIF
  -> R5 (KS1.3): the probe writer must GO before the child arrives -
  -> PIPE: pairs one writer with one reader, and the child brings its
  -> own writer through the runasync script's redirection
  Close(wout)
  wout := NIL
  IF dir
    dlock := Lock(dir, SHARED_LOCK)
    IF dlock THEN old := CurrentDir(dlock)
  ENDIF
  res := runasync(cmd, 'PIPE:cfile-arc')
  IF res = -1
    IF dlock
      CurrentDir(old)
      UnLock(dlock)
    ...
    RETURN -1
  ENDIF
  -> R5: the CWD lock stays held until the pipe's EOF proves the
  -> child exited - 1.3 RUN shares the lock, nothing refcounts it
  -> for the child the way SYS_ASYNCH's DupLock did
  ...
  ENDIF
  IF dlock    -> EOF seen (or no reader): safe to let the CWD go now
    CurrentDir(old)
    UnLock(dlock)
  ENDIF
ENDPROC res
...
PROC arcaddstaged(p, stageroot)
  ...
  IF islzx(p)
    StringF(base, 'lzx -M64 -bi8 -bo16 -r -e a "\s"', arcpath[p])
  ELSE
...
-> instantly). Run completion is unambiguous: the child's CLI owns
-> the capture file handle, so an EXCLUSIVE Lock on it succeeds only
-> after the archiver exits. idxp/credp persist the cursor across
-> BATCHED runs (arcextracttree splits big trees over several
-> commands). R5 (KS1.3): the detach is runasync (C:Run + a script
-> redirect); the file is NOT pre-created here - until the child's
-> CLI opens it the Lock finds nothing, which reads correctly as
-> "still running". Esc (b21, weakened on 1.3): RUN names its child
-> itself, so FindTask(arcname()) misses and arcbreak is a no-op -
-> Esc can no longer stop a running archiver, but the caller's abort
-> flag still keeps the partial output from landing (the stage wipe
-> eats it) once the run ends on its own.
PROC arcsizeof(path)
  DEF lock, fib:PTR TO fileinfoblock, n=-1
  IF (fib := allocfib()) = NIL THEN RETURN -1
  IF lock := Lock(path, SHARED_LOCK)
...
PROC arcpollrun(cmd, files:PTR TO LONG, sizes:PTR TO LONG, nf,
  ...
  DEF res, lk, sz, done=FALSE, adv, d, killed=FALSE
  DeleteFile('T:CFile-out')
  res := runasync(cmd, 'T:CFile-out')
...
PROC arcsideindirect(p, q, nm, member, ismove)
  ...
  ENDIF
  IF islzx(q)
    StringF(cmd, 'lzx -M64 -bi8 -bo16 -r -e a "\s" "\s"', arcpath[q], topname)
  ELSE
    ...
    IF pick AND (stop = FALSE)
      IF edirs[b + i]
        ...
        IF EstrLen(arcsub[q]) = 0
          ...
          IF islzx(q)
            StringF(cmd, 'lzx -M64 -bi8 -bo16 -r -e a "\s" "\s"', arcpath[q], nm)
          ELSE
...
PROC doxfer(ismove, force)
  ...
  IF la
    IF lb
      -> R8: SameLock < 0 meant LOCK_DIFFERENT (other volume); the
      -> fl_Task compare answers the same-volume question directly
      IF locksamevol(la, lb) = FALSE THEN samevol := FALSE
    ENDIF
  ENDIF
  ...
  cancelok := FALSE
  progoff()
  refreshpanes(showbar)    -> CFile13: not refreshall - see refreshpanes
  IF abort
...
PROC delone(p, i)
  ...
  -> CFile13: mounted-image release cut (no DAControl on KS1.3)
  IF edirs[i]
...
-> icon.library, the i window's .info reader - lazily opened;
-> absent = the icon rows say so, all else works
PROC iconopen()
  IF iconbase = NIL
    IF icontried = FALSE
      icontried := TRUE
...
-> live via SetProtection, Esc closes. Since 0.5b48 it also reads
-> the .info sidecar DOpus-style:
...
PROC infowindow()
  ...
  -> R6 (KS1.3): the own formatter replaces DateToStr, same strings
  ds := fib.datestamp
  dsdatestr(ds[0], db)
  dstimestr(ds[1], ds[2], tb)
  StrCopy(cmt, fib.comment)
  freefib(fib)
  -> CFile13: datatypes identification cut (KS1.3 has no datatypes.library)
  StrCopy(tyb, '-')
...
-> sniff so a caller that already read the file can type it
...
PROC viewfile(path, name, mode, bulk)
  ...
  ELSE
    ...
    len := size
    vbsize := size
    vbcap := IF size < vbwinsz THEN size ELSE vbwinsz
...
PROC bulkview(p)
  ...
  WHILE pos >= 0
    ...
    -> CFile13: datatypes picture tour cut (KS1.3 has no datatypes.library)
...
-> CFile13: datatypes viewing/playing and mod playback cut (KS1.3 has no
-> datatypes.library, and ptreplay went with the media verbs); pictures,
-> sounds and mods fall through to the hex viewer.
...
PROC doview()
  ...
  ELSE
    -> CFile13: datatypes/mod viewing cut (KS1.3) - the rest hex-views
...
PROC edload(path)
  ...
  IF edl = NIL
    -> CFile13 (0.1b14): the table starts small on a small machine
    -> - edensure doubles it on demand (its old jump-to-EDMAXL
    -> minimum retuned to match)
    edcap := IF AvailMem(0) < $100000 THEN 1024 ELSE EDMAXL
    IF (edl := New(Mul(edcap, 4))) = NIL THEN RETURN FALSE
  ENDIF
  edfree()
  ednew := FALSE
  IF fib := allocfib()
...
PROC edsave(path)
  ...
  IF samefile(path, 'S:cfile13.config') THEN wantreload := TRUE
        ...
        IF edmod
          ...
          IF (k = "y") OR (k = "Y")
            IF edsave(path)
              ...
              done2 := TRUE
            ELSE
              -> CFile13 (0.1b8): edsave's fault used to be repainted
              -> over by edtitle within the same frame - a failed save
              -> (a read-only 1.3 floppy, say) looked like a silent
              -> nothing, "still *modified*" (his find). Hold the
              -> fault on screen until a key says it was read.
              waitvanilla()
            ENDIF
          ELSEIF (k = "n") OR (k = "N")
            done2 := TRUE
...
PROC arcreadd(p, scratchroot, member)
  ...
  IF islzx(p)
    lzxdelmember(arcpath[p], member)
    StringF(cmd, 'lzx -M64 -bi8 -bo16 -r -e a "\s" "\s"', arcpath[p], topname)
  ELSE
    arcdelmember(arcpath[p], member)
...
-> (so launched programs find their data files). R5 (KS1.3): the
-> 2.0 console options died with System() - AUTO/CLOSE/WAIT/SCREEN
-> are all V36 con-handler features, and without pubscreens the
-> window cannot visit our screen at all. So: a plain 1.3 console
-> on the WORKBENCH screen (brought to front for the run), Execute
-> synchronously with its output there, and a "press RETURN" read
-> standing in for WAIT so the output can actually be read. The
-> command's return code is not visible through Execute (V33) -
-> errors show in the console output itself, so nothing is said
-> when the run at least started (the conservative branch).
PROC runcmd(p, cmd)
  DEF dlock=NIL, old=NIL, res=-1, coni=NIL, s:PTR TO CHAR, kb[4]:ARRAY OF CHAR
  dlock := Lock(ppath[p], SHARED_LOCK)
  IF dlock THEN old := CurrentDir(dlock)
  coni := Open('CON:0/0/640/200/CFile', OLDFILE)
  IF ownscr THEN WbenchToFront()    -> the console lives back there
  IF coni
    IF Execute(cmd, NIL, coni) THEN res := 0
    s := '\n** press RETURN to continue **'
    Write(coni, s, StrLen(s))
    Read(coni, kb, 1)    -> CON: is line-buffered: RETURN releases it
  ELSE
    -> no console to be had: run blind, the refresh shows the effect
    IF Execute(cmd, NIL, NIL) THEN res := 0
  ENDIF
  IF coni THEN Close(coni)
  IF ownscr THEN ScreenToFront(scr)
  ActivateWindow(win)
  IF dlock
    CurrentDir(old)
    UnLock(dlock)
  ...
  IF res = -1 THEN faultmsg('cannot run')
ENDPROC
...
PROC capturecmd(p, cmd, title, refresh)
  ...
  IF (fout := Open('T:CFile-out', NEWFILE)) = NIL
    ...
    RETURN
  ENDIF
  -> R5 (KS1.3): Execute, synchronous, output captured. The command's
  -> return code is invisible on 1.3 - the captured output the viewer
  -> shows right below carries any error text, so the old "returned
  -> code n" note is simply dropped (the conservative branch).
  IF Execute(cmd, NIL, fout) THEN res := 0
  Close(fout)
  IF dlock
    CurrentDir(old)
...
PROC connl()
  ...
  ENDIF
  IF cmodel
    IF cmrow < (cmlines - 1)    -> past the cap the last line churns
      cmrow := cmrow + 1
...
-> model (allocation failed) or a full one (the cmlines cap - the old
  ...
  WHILE (i < n) AND (cmfull = FALSE)
    ...
    ELSE                -> done, the direct path takes the rest
      ...
      ELSEIF c >= 32
        ...
        ENDWHILE
        run := j - i
        WHILE (run > 0) AND (cmrow < (cmlines - 1))
...
PROC livestart()
  ...
  -> M1 (the 1MB pass): CMAXL x ncols was 320K on the first ':' -
  -> deadly on a 1MB machine. The copybuf AvailMem ladder: the full
  -> 4000 lines only when RAM clearly allows, 500 otherwise (CMAXL
  -> stays the ceiling constant). New() raw bytes, sized here because
  -> ncols is only known once the grid exists.
  IF cmodel = NIL
    cmlines := IF AvailMem(0) > $400000 THEN CMAXL ELSE 500
    IF AvailMem(0) < $100000 THEN cmlines := 100    -> 0.1b14/b17: 512K rung, 8K model
    cmodel := New(Mul(cmlines, ncols))
  ENDIF
  cmrow := 0
  cmfull := FALSE
  IF cmodel
...
PROC livepipe(p, cmd)
  DEF dlock=NIL, old=NIL, res, wout=NIL, rdr=NIL,
      buf:PTR TO CHAR, n, s:PTR TO CHAR
  buf := pipebuf    -> I7a: 4KB shared pipe buffer, not 256B on the stack
  IF (wout := quietopen('PIPE:cfile-con', NEWFILE)) = NIL
    -> CFile13 (0.1b11): a stock 1.3 has no PIPE:, and this used to
    -> RETURN FALSE - which dopack/dounpack read as "stop", so the
    -> archiver never ran AT ALL (his pack test: empty console, no
    -> disk activity). The live road has a batch twin: run the
    -> command synchronously with output captured to T:, then pour
    -> the capture through the same console feed - everything the
    -> archiver said arrives after the fact instead of during.
    res := runcapture(ppath[p], cmd, 'T:CFile-con')
    IF res = -1
      s := 'cannot run the command\n'
      confeed(s, StrLen(s))
      RETURN TRUE
    ENDIF
    IF rdr := Open('T:CFile-con', OLDFILE)
      n := Read(rdr, buf, PIPESZ)
      WHILE n > 0
        confeed(buf, n)
        n := Read(rdr, buf, PIPESZ)
      ...
      Close(rdr)
    ENDIF
    DeleteFile('T:CFile-con')
    RETURN TRUE
  ENDIF
  -> R5 (KS1.3): the probe writer must go before the child arrives -
  -> PIPE: pairs one writer with one reader, and the child brings
  -> its own writer through the runasync script's redirection
  Close(wout)
  dlock := Lock(ppath[p], SHARED_LOCK)
  IF dlock THEN old := CurrentDir(dlock)
  res := runasync(cmd, 'PIPE:cfile-con')    -> it runs while we render
  IF res = -1
    IF dlock
      CurrentDir(old)
      UnLock(dlock)
  ...
  IF rdr := Open('PIPE:cfile-con', OLDFILE)
    ...
    Close(rdr)
  ENDIF
  -> R5: the CWD lock is held to the pipe's EOF - 1.3 RUN shares it
  -> with the child, and nothing refcounts it on the child's behalf
  IF dlock
    CurrentDir(old)
    UnLock(dlock)
  ENDIF
...
PROC liveend()
  DEF s:PTR TO CHAR, vtop, maxv, nv, r, line,
...
-> and be a directory, canonicalised via locknamefrom (device -> volume name,
...
PROC gotopath(p, path)
  ...
  ELSE
    ...
    IF isdir = FALSE
      ...
      RETURN FALSE
    ENDIF
    IF locknamefrom(lock, canon) = FALSE THEN StrCopy(canon, path)
    UnLock(lock)
...
-> bookmark rule - container interiors are lies once
-> the archive closes), most-recent-first with
...
PROC dofind()
  DEF p, res:PTR TO LONG, cnt=0, i, pat[42]:STRING, pick, pp:PTR TO CHAR,
  ...
  IF efail[p] THEN RETURN
  StrCopy(pat, '')
  IF lineinput('find (name): ', pat, 40, FALSE) = 0
    drawpaths()
    RETURN
  ENDIF
  IF EstrLen(pat) = 0
  ...
  IF (res := New(FINDMAX * 4)) = NIL
    ...
    RETURN
  ENDIF
  -> CFile13: the #? glob road cut with pattern marking - find is a
  -> case-insensitive substring match
  showmsg('searching - Esc to stop')
  cancelok := TRUE
  abort := FALSE
  findwalk(ppath[p], 0, pat, res, {cnt})
  cancelok := FALSE
  abort := FALSE
  IF cnt = 0
    Dispose(res)
...
-> CFile13: recursive grep (pane t search) cut - scanbuf-hungry for a 512K
-> target; the viewer's internal / find stays
...
PROC dopack()
  ...
  ELSEIF ty = 2
    StringF(cmd, 'lzx -M64 -bi8 -bo16 -r a "\s"', dst)
  ELSE
...
-> CFile13: mark-by-pattern (+) cut - ParsePattern/MatchPattern go
-> unless a tiny own matcher earns its bytes; a/A/* stay
...
PROC donew()
  ...
  IF efail[p] THEN RETURN
  StrCopy(tname, '')
  IF lineinput('new (/ = dir): ', tname, 31, FALSE) = 0
    drawpaths()
    RETURN
  ENDIF
  l := EstrLen(tname)
  ...
  IF wantdir
    ...
    ENDIF
  ELSE
    -> CFile13: blank-ADF creation cut (no DAControl on KS1.3) - a name
    -> ending .adf is just a normal new file
    IF pathtype(dpath) > 0
...
PROC helpscreen()
  ...
  lines := [vertitle,
            ...
            'f .......... find by name, recursive (substring match)',
            ...
            'Enter ...... open by type: dir/archive/image, view, run',
            'v .......... view text/ANSI/hex',
...
-> CFile13 (0.1b16): the BENCH instrument stripped - dev scaffolding
-> (its own comment said so); the perf story it measured lives on in
-> perf-roadmap.md and the main line.
...
version: CHAR '$VER: CFile13 0.1b17 (1.8.26) E build',0
