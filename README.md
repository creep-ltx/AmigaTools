...
| [ccon](ccon/) | `CCON:` — a console handler, the CON:/KingCON class: a mounted DOS handler speaking the packet protocol, hosting a real shell (`NewShell CCON:`) with output scrollback (the one thing stock CON: cannot be given from outside), a modern line editor with history/ghosts/completion/Ctrl+R, iconify-to-Workbench, an alternate-screen contract for Ed and More, KingCON-style `CON:`/`RAW:` takeover — and, since 1.2.2, a model-first render engine that is 4× faster than stock CON: under render barriers. Current release: [1.2.2](../../releases/tag/ccon-1.2.2). |
