...
| [ccon](ccon/) | `CCON:` — a console handler, the CON:/KingCON class: a mounted DOS handler speaking the packet protocol, hosting a real shell (`NewShell CCON:`) with output scrollback (the one thing stock CON: cannot be given from outside), a modern line editor with history/ghosts/completion/Ctrl+R, iconify-to-Workbench, an alternate-screen contract for Ed and More, KingCON-style `CON:`/`RAW:` takeover — and, since 1.2.2/1.2.3, a model-first, plane-masked render engine that makes it the fastest console of the three measured (stock CON:, ViNCEd) with render barriers on, at accelerated and stock-A1200 timing alike. Current release: [1.2.3](../../releases/tag/ccon-1.2.3). |
