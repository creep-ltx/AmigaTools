...
| [cfile](cfile/) | Two-pane keyboard-driven text-mode file manager — copy/move/delete/rename with marks and collision prompts, recursive directory operations with a progress bar, per-row size column with on-demand directory measuring, free-space and marked totals, volume list, text/ANSI/hex viewer, built-in text editor, browse and edit inside lha archives, archive packing and unpacking, live in-frame console with scrollback, protection-bit editor, shell commands, and a config file with custom fonts, live reload and remembered pane paths. |
